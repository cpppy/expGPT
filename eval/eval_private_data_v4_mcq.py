...
class AnswerBot:
    ...
    def completion_inference(self, prompt):
        ...
        # response = outputs[0][input_ids.shape[-1]:]
        response_str = self.tokenizer.decode(outputs[0][input_ids.shape[-1]:], skip_special_tokens=True)
        print(response_str)
        exit(0)
        # response_str = response_str[0:8].strip()
        return response_str
...
def create_optimized_prompt(row):
    ...
    prompt = (
        # f"Read the question and choose the correct option based on the options given.\n\n"
        f"Question: {row['question']}\n"
        f"Options:\n"
        f"A. {row['options']['A']}\n"
        f"B. {row['options']['B']}\n"
