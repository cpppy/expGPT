...
class Dataset2Lmdb(object):
    ...
    def __init__(self, dataset, lmdb_path, num_workers=4):
    ...
    def generate_for_text(self, write_frequency=5000):
        ...
        for idx, batch in enumerate(self.data_loader):
            ...
            txn.put(u'{}'.format(idx).encode('ascii'), self.dumps_pyarrow(sample_data))
            if idx % write_frequency == 0:
                print("[%d/%d]" % (idx, len(self.data_loader)))
                txn.commit()
                txn = db.begin(write=True)
        ...
        keys = [u'{}'.format(i).encode('ascii') for i in range(len(self.dataset))]
        with db.begin(write=True) as txn:
            txn.put(b'__keys__', self.dumps_pyarrow(keys))
            txn.put(b'__len__', self.dumps_pyarrow(len(keys)))
...
def trans_dataset_to_lmdb():
    ...
    train_dataset, eval_dataset = build_dataset(tokenizer=tokenizer, eval_ratio=0.1)

    # ds2lmdb_tool = Dataset2Lmdb(dataset=train_dataset,
    #                             lmdb_path='/mnt2/data/dsp_data_files2/zhihukol_train_tokenizerQwen2_cache_20241126.lmdb',
    #                             num_workers=4)

    ds2lmdb_tool = Dataset2Lmdb(dataset=eval_dataset,
                                lmdb_path='/mnt2/data/dsp_data_files2/zhihukol_eval_tokenizerQwen2_cache_20241126.lmdb',
                                num_workers=4)
