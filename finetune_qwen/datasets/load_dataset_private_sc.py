...
def make_supervised_data_module(
    ...
    for _data in texts:
        ...
        try:
            ...
            # TODO: build QA PromptTemplate or regenerate QA samples

            question = _data['question']
            options = '\n'.join([f'{k}: {v}' for k, v in _data['options'].items()])
            query = PromptTemplate.replace('${question}', question).replace('${options}', options)
            messages.append({"from": "user", "value": query})
            messages.append({
                "from": "assistant",
                "value": _data['options'][_data['answer']] + '。\n分析：\n' + _data['explaination']
            })
...
class DataArguments:
    ...
    data_path = '/data/data/med_dataset/llm_gen/samples_from_baiduyidian_20240919.jsonl'
    eval_data_path=None
