...
#                 _target[0: len(tokenizer(role).input_ids + nl_tokens)] = [IGNORE_TOKEN_ID] * len(
...
def preprocess_fixed_dpo(
    ...
    for i, source in enumerate(sources):
        ...
        for branch in ['chosen', 'rejected']:
            sentences = history + source[branch]
            # if roles[source[0]["from"]] != roles["user"]:
            #     source = source[1:]
            ...
            _processed[branch] = dict(
                input_ids=input_ids,
                labels=labels,
                # attention_mask=[[]] * len(input_ids),
                attention_mask=[],
            )
        processed.append(_processed)
...
class RawReaderDPO:
    ...
    def __init__(self, raw_dir, pattern='*.*'):
        self.raw_dir = raw_dir
        self.pattern = pattern

    def _load_samples(self):
        files = glob.glob(f'{self.raw_dir}/{self.pattern}')
        samples = []
        for file in files:
            _samples = []
            with jsonlines.open(file=file) as f:
                for s in f.iter():
                    # print(s)
                    # exit(0)
                    _samples.append(dict(
                        ...
                        history=s['history'],
                        question=s['question'],
                        response_chosen=s['response_chosen'],
                        response_rejected=s['response_rejected']
                    ))
            print(f'[DATASET] {len(_samples)} from file: {file}')
            samples.extend(_samples)
        print(f'[DATASET] Total {len(samples)} samples from dir: {self.raw_dir}')
        return samples

    def _format_samples(self, samples):
        formatted_samples = []
        for s in samples:
            formatted_samples.append(dict(
                chosen=[
                    {
                        "role": "user",
                        "content": s['question']
                ...
                rejected=[
                    ...
                    {
                        "role": "assistant",
                        "content": s['response_rejected']
                    }
                ],
            ))
        return formatted_samples

    def get_formatted_samples(self):
        _samples = self._load_samples()
        _formatted_samples = self._format_samples(_samples)
        return _formatted_samples
...
class RawReaderDPO3:
    ...
    def _load_samples(self):
        ...
        for file in files:
            ...
            # print(json.dumps(raw_datas[0], indent=4, ensure_ascii=False))
            # exit(0)

            _samples = []
            for s in raw_datas:
                chat_history = s['conversations'][:-1]
                chat_history = [
                    dict(
                        role=dict(human='user', gpt='assistant', system='system')[m['from']],
                        content=m['value']
                    )
                    for m in chat_history
                ]
                new_message = s['conversations'][-1]
                _samples.append(dict(
                    # system='',
                    history=chat_history,
                    question=new_message['value'],
                    response_chosen=s['chosen']['value'],
                    response_rejected=s['rejected']['value'],
                ))
            print(f'[DATASET] {len(_samples)} from file: {file}')
            samples.extend(_samples)
        print(f'[DATASET] Total {len(samples)} samples from dir: {self.raw_dir}')
...
class CustomDPODataset(Dataset):
    ...
    def __init__(self,
        ...
        from functools import partial
        preprocess_partial = partial(preprocess_fixed_dpo,
                                     max_len=-1,
                                     pad_and_clip=False,
                                     output_tensor=False)
        ...
        with Pool(processes=PROC_NUM) as pool:
            ...
            for i in range(PROC_NUM):
                if i < PROC_NUM - 1:
                    _samples = samples[i * n_split: (i + 1) * n_split]
                else:
                    _samples = samples[i * n_split:]
                # result = pool.apply_async(preprocess_partial, args=(sample,), callback=update_progress)
                _results = pool.apply_async(preprocess_partial, args=(_samples, i))
                apply_results.append(_results)
        ...
        # self.labels = [x["labels"] for x in results]
        # self.attention_mask = [x["attention_mask"] for x in results]
        self.dpo_samples = results

    def __len__(self):
        return len(self.dpo_samples)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor] or Dict[str, List]:
        ...
        #     attention_mask=self.attention_mask[i],
        # )
        return self.dpo_samples[i]


if __name__ == '__main__':
    # raw_reader = RawReaderDPO(raw_dir='../../data', pattern='*.jsonl')

    # raw_reader = RawReaderDPO2(raw_dir='/data/data/llm_data/orpo-dpo-mix-40k/data', pattern='*.parquet')
    # formatted_samples = raw_reader.get_formatted_samples()
    # exit(0)

    raw_reader = RawReaderDPO3(raw_dir='/data/data/llm_data/DPO-En-Zh-20k', pattern='*en.json')
    formatted_samples = raw_reader.get_formatted_samples()
    # exit(0)

    dataset = CustomDPODataset(samples=formatted_samples, datasetname='dpo_zh_500')
